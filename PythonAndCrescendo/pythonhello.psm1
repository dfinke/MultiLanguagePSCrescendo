...
PROCESS {
    ...
    if ( $PSBoundParameters["Verbose"]) {
         Write-Verbose -Verbose -Message $PSScriptRoot/hello.py
    ...
    if ( $PSCmdlet.ShouldProcess("$PSScriptRoot/hello.py")) {
        if ( $__handlerInfo.StreamOutput ) {
            & "$PSScriptRoot/hello.py" $__commandArgs | & $__handler
        }
        else {
            $result = & "$PSScriptRoot/hello.py" $__commandArgs
            & $__handler $result
        }
    }
  } # end PROCESS
...
<#
.SYNOPSIS
Could not find $PSScriptRoot/hello.py to generate help.

.DESCRIPTION See help for $PSScriptRoot/hello.py
...
Export-ModuleMember -Function Get-HelloPython
